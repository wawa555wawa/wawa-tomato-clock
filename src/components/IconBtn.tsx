interface Props {
  icon: 'gear' | 'stats' | 'close' | 'reset' | 'play' | 'pause' | 'skip'
  label: string
  onClick?: () => void
  className?: string
  disabled?: boolean
  ...
  return (
    ...
    >
      <svg viewBox="0 0 24 24" width="22" height="22" aria-hidden>
        {icon === 'reset' ? (
          <path d={PATHS.reset} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        ) : icon === 'stats' ? (
          <path d={PATHS.stats} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        ) : (
